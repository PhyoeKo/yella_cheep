...
class AddPlaceScreenState extends State<AddPlaceScreen> {
  ...
  TextEditingController howToReachValueController = TextEditingController();
  ...
  String? howToReachKey;
  HowToReachModel? howToReachModel;
  ...
  void init() async {
    ...
    if (isUpdate) {
      ...
      emailController.text = widget.placeModel!.email.validate();
      phoneController.text = widget.placeModel!.phone.validate();
      websiteController.text = widget.placeModel!.website.validate();
      attractionController.text = widget.placeModel!.attractions.validate();
      fromMonth = widget.placeModel!.fromMonth;
      toMonth = widget.placeModel!.toMonth;
      howToReach = widget.placeModel!.howToReach ?? [];
    }
    setState(() {});
  }
  ...
  Future<void> save() async {
    ...
    placeModel.website = websiteController.text;
    placeModel.email = emailController.text;
    placeModel.phone = phoneController.text;
    placeModel.fromMonth = fromMonth;
    placeModel.toMonth = toMonth;
    placeModel.attractions = attractionController.text;
    placeModel.howToReach = howToReach;
            ...
            child: Form(
              ...
              child: Column(
                ...
                children: [
                  ...
                  30.height,
                  Text(language.placesHistory, style: boldTextStyle()),
                  24.height,
                  Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(language.website, style: primaryTextStyle()),
                      8.height,
                      AppTextField(
                        ...
                        decoration: commonInputDecoration(hintText: language.website),
                        validator: (s) {
                          if (s!.isNotEmpty && !s.validateURL()) {
                  ...
                  Column(
                    ...
                    children: [
                      ...
                      AppTextField(
                        ...
                        validator: (s) {
                          if (s!.isNotEmpty && !s.trim().validateEmail()) return language.emailIsInvalid;
                          return null;
                        },
                      ),
                    ],
                  ...
                  Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(language.phone, style: primaryTextStyle()),
                      8.height,
                      AppTextField(
                        ...
                        errorThisFieldRequired: errorThisFieldRequired,
                        decoration: commonInputDecoration(hintText:language.phone),
                  ...
                  Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(language.bestTimeToVisit, style: primaryTextStyle()),
                      8.height,
                      Row(
                        children: [
                          DropdownButtonFormField<String>(
                            ...
                            value: fromMonth,
                            decoration: commonInputDecoration(),
                            items: monthList().map<DropdownMenuItem<String>>((item) {
                              return DropdownMenuItem(
                                value: item,
                                child: Text(item.validate(), style: primaryTextStyle()),
                              );
                            }).toList(),
                            onChanged: (value) {
                              fromMonth = value!;
                            ...
                            validator: (s) {
                              if(s==null && toMonth!=null) return language.selectFromMonth;
                              return null;
                            },
                          ).expand(),
                          16.width,
                  ...
                  Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(language.attractions, style: primaryTextStyle()),
                      8.height,
                      AppTextField(
                        ...
                        errorThisFieldRequired:errorThisFieldRequired,
                        decoration: commonInputDecoration(hintText: language.attractions),
                      ),
                    ],
                  ),
                  16.height,
                  ...
                  Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(language.howToReach, style: primaryTextStyle()),
                      8.height,
                      Row(
                        children: [
                          DropdownButtonFormField<String>(
                            ...
                            decoration: commonInputDecoration(),
                            value: howToReachKey,
                            items: howToReachList().map<DropdownMenuItem<String>>((item) {
                              return DropdownMenuItem(
                                value: item,
                                child: Text(item.validate(), style: primaryTextStyle()),
                              );
                            ...
                            onChanged: (value) {
                              howToReachKey = value;
                              if (howToReach.any((element) => element.key == howToReachKey)) {
                                howToReachModel = howToReach.firstWhere((element) => element.key == howToReachKey);
                                howToReachValueController.text = howToReachModel!.value.validate();
                              } else {
                                howToReachModel = null;
                                howToReachValueController.clear();
                              }
                              setState(() {});
                            },
                            validator: (s) {
                          ...
                          16.width,
                          dialogPrimaryButton(howToReachModel != null ? language.update : language.add, () {
                            bool isExist = howToReach.any((element) => element.key == howToReachKey);
                            if (isExist) {
                              howToReach.remove(howToReachModel);
                            }
                            if(howToReachKey!=null) {
                              howToReach.add(HowToReachModel(key: howToReachKey, value: howToReachValueController.text));
                              howToReachKey = null;
                              howToReachValueController.clear();
                            }
                            setState(() {});
                      ...
                      16.height,
                      AppTextField(
                        controller: howToReachValueController,
                        autoFocus: false,
                        textFieldType: TextFieldType.OTHER,
                        keyboardType: TextInputType.text,
                        errorThisFieldRequired: errorThisFieldRequired,
                        decoration: commonInputDecoration(hintText: language.howToReach),
                      ),
                    ],
                  ),
                  16.height.visible(howToReach.isNotEmpty),
                  ListView.builder(
                    shrinkWrap: true,
                    itemCount: howToReach.length,
                    itemBuilder: (context, index) {
                      HowToReachModel item = howToReach[index];
                      return RichText(
                        text: TextSpan(
                          text: "${item.key}:\t",
                          style: boldTextStyle(),
